...
const HistoryTab: React.FC = () => {
    const { history, deleteFromHistory, clearHistory, isLoading, error, refreshHistory } = useHistory();
    ...
    return (
        <div className="flex h-[calc(100vh-140px)] gap-6">
            ...
            <aside className="w-1/4 min-w-[300px] flex flex-col bg-[#161d2f]/50 border border-white/5 rounded-[2rem] overflow-hidden">
                <div className="p-6 border-b border-white/5 flex justify-between items-center bg-[#161d2f]">
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            className={`text-slate-500 hover:text-white transition-colors ${isLoading ? 'animate-spin' : ''}`}
                ...
                {error && (
                    <div className="p-4 m-4 bg-rose-500/10 border border-rose-500/20 rounded-xl text-xs text-rose-300">
                        {error}
                    </div>
                )}
