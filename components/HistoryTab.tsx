...
const HistoryTab: React.FC = () => {
    const {
        history, images, videos, batches,
        deleteFromHistory, clearHistory,
        deleteImage, deleteVideo, deleteBatch,
        isLoading, error, refreshHistory
    } = useHistory();

    const [activeSubTab, setActiveSubTab] = useState<'scans' | 'gallery' | 'videos' | 'batches'>('scans');
    ...
    const GalleryView = () => (
        <div className="animate-in fade-in duration-300">
            {images.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-[50vh] text-slate-500">
                    <i className="fa-regular fa-images text-4xl mb-4 text-slate-600"></i>
                    <h3 className="text-xl font-bold text-slate-400">No Generated Images</h3>
                    <p className="text-sm">Generate some images in the Studio to see them here.</p>
                </div>
            ) : (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-4">
                    {images.map((img) => (
                        <div key={img.filename} className="group relative bg-[#161d2f] rounded-[2rem] overflow-hidden border border-white/5 hover:border-sky-500/30 transition-all">
                            <div className="aspect-square bg-[#0a0f1d] relative">
                                <img
                                    src={img.url}
                                    alt={img.filename}
                                ...
                                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                    ...
                                    <button
                                        onClick={() => { if (window.confirm('Delete this image?')) deleteImage(img.filename); }}
                                        className="p-3 rounded-full bg-rose-500/20 hover:bg-rose-500 text-rose-200 hover:text-white backdrop-blur-md transition-colors"
                                        title="Delete"
                                    >
                            ...
                            <div className="p-4">
                                ...
                                <p className="text-xs font-bold text-slate-300 truncate" title={img.filename}>
                                    {img.filename}
                                </p>
                            </div>
                        </div>
    ...
    const VideosView = () => (
        <div className="animate-in fade-in duration-300">
            {videos.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-[50vh] text-slate-500">
                    <i className="fa-solid fa-film text-4xl mb-4 text-slate-600"></i>
                    <h3 className="text-xl font-bold text-slate-400">No Generated Videos</h3>
                    <p className="text-sm">Generate videos to see them here.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
                    {videos.map((vid) => (
                        <div key={vid.filename} className="group relative bg-[#161d2f] rounded-[2rem] overflow-hidden border border-white/5 hover:border-violet-500/30 transition-all">
                            <div className="aspect-video bg-[#0a0f1d] relative">
                                <video
                                    src={vid.url}
                                ...
                                />
                                <button
                                    onClick={() => { if (window.confirm('Delete this video?')) deleteVideo(vid.filename); }}
                                    className="absolute top-2 right-2 p-2 rounded-full bg-black/50 text-rose-400 hover:bg-rose-500 hover:text-white transition-all opacity-0 group-hover:opacity-100"
                                >
                            ...
                            <div className="p-4">
                                ...
                                <p className="text-xs font-bold text-slate-300 truncate" title={vid.filename}>
                                    {vid.filename}
                                </p>
                            </div>
                        </div>
        ...
        <div className="animate-in fade-in duration-300">
            {batches.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-[50vh] text-slate-500">
                    ...
                    <h3 className="text-xl font-bold text-slate-400">No Prompt Batches</h3>
                    <p className="text-sm">Generated prompt batches will appear here.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
                    {batches.map((batch) => (
                        <div key={batch.filename} className="bg-[#161d2f] p-6 rounded-[2rem] border border-white/5 hover:border-emerald-500/30 transition-all group relative">
                            <button
                                onClick={() => { if (window.confirm('Delete this batch?')) deleteBatch(batch.filename); }}
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    <h4 className="font-bold text-slate-200 truncate max-w-[180px]" title={batch.filename}>{batch.filename}</h4>
                                    <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest">{new Date(batch.timestamp).toLocaleDateString()}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-2">
                                <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="flex-1 py-3 rounded-xl bg-[#0a0f1d] hover:bg-emerald-500 hover:text-white border border-white/5 text-center text-xs font-bold transition-all"
                                >
                                    View JSON
                                </a>
                                <a
                                    ...
                                    className="px-4 py-3 rounded-xl bg-[#0a0f1d] hover:bg-emerald-500 hover:text-white border border-white/5 text-center text-xs font-bold transition-all"
                                >
                                    <i className="fa-solid fa-download"></i>
                                </a>
                            </div>
        ...
        <div className="h-full flex flex-col">
            ...
            <div className="flex items-center justify-between px-8 py-6 border-b border-white/5">
                <div className="flex items-center gap-4">
                    <h2 className="text-2xl font-black uppercase italic tracking-tighter text-white">Asset Gallery</h2>
                    <button
                        onClick={refreshHistory}
                        disabled={isLoading}
                        className={`w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-slate-400 transition-all ${isLoading ? 'animate-spin' : ''}`}
                    >
                ...
                <div className="flex bg-[#0a0f1d] p-1 rounded-xl border border-white/5">
                    {(['scans', 'gallery', 'videos', 'batches'] as const).map((tab) => (
                        <button
                            key={tab}
                            onClick={() => setActiveSubTab(tab)}
                            className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeSubTab === tab
                                    ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20'
                                    : 'text-slate-500 hover:text-slate-300'
                                }`}
                        >
                            {tab === 'gallery' ? 'Images' : tab.charAt(0).toUpperCase() + tab.slice(1)}
                        </button>
                    ))}
                </div>
            </div>
            ...
            {/* Error Message */}
            {error && (
                <div className="mx-8 mt-4 p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl text-sm text-rose-300 flex items-center gap-3">
            ...
            <div className="flex-1 overflow-hidden p-8">
                {activeSubTab === 'scans' && <ScansView />}
                {activeSubTab === 'gallery' && <GalleryView />}
                {activeSubTab === 'videos' && <VideosView />}
                {activeSubTab === 'batches' && <BatchesView />}
            </div>
        </div>
    );
};
