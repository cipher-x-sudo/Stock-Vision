...
// Load .env from repo root (Stock-Vision) then app root so API keys from project .env are used
...
// ── Health (for Nexus Studio Settings) ───────────────────────
...
if (fs.existsSync(distPath)) {
  ...
  app.get("/", (req, res) => {
    res.type("html").send(`
<!DOCTYPE html><html><head><meta charset="utf-8"><title>Nexus Studio</title></head><body style="font-family:sans-serif;max-width:520px;margin:60px auto;padding:20px;background:#0a0f1d;color:#e2e8f0;">
  <h1 style="color:#0ea5e9;">Nexus Studio</h1>
  <p>Backend is running. The frontend has not been built yet.</p>
  <p>From the app root run:</p>
  <pre style="background:#161d2f;padding:12px;border-radius:8px;overflow:auto;">npm run build</pre>
  <p>Then restart the app (<code>npm start</code>).</p>
</body></html>`);
  });
}

// ── Flow (Veo4K) backend + start server ───────────────────────
process.env.APP_ROOT = ROOT;
process.env.NEXUS_STUDIO_MERGED = "1";
(async () => {
  try {
    const flow = await import("./flow/index.js");
    const flowState = flow.createFlowState();
    app.state = flowState;
    flow.registerRoutes(app);
    setImmediate(() => {
      flow.initBackend(flowState).catch((e) => console.error("[Flow] init error:", e.message || e));
    });
    console.log("[Flow] Routes mounted at /api/flow/*");
  } catch (e) {
    console.warn("[Flow] Could not load Veo4K backend:", e.message || e);
  }
  app.listen(PORT, () => {
    console.log(`Nexus Studio backend listening on http://localhost:${PORT}`);
    if (!getCookieHeader()) {
